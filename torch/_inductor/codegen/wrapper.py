...
class CppWrapperCodeGen(WrapperCodeGen):
    ...
    def generate_extern_kernel_args_decl_if_needed(
        ...
        def fill_args(arg, arg_type):
            ...
            if isinstance(arg_type, torch.TensorType):
                ...
                # List[Optional[Tensor]]
                    ...
                    new_tensor_args.extend([f"{a.name}.get()" for a in arg if a is not None])
        ...
        def fill_output_arg(arg, return_type):
            if isinstance(return_type, torch.TensorType):
                ...
                self.writeline(
                    f"AOTI_TORCH_ERROR_CHECK(aoti_torch_new_tensor(&{arg}_handle));")
                self.writeline(f"auto {arg} = create_raii_tensor_handle_for_temp({arg}_handle);")
    ...
    def generate_extern_kernel_alloc_and_find_schema_if_needed_fbcode(
        ...
        self.writeline(f"AtenTensorHandle {tensor_args_var}[] = {{{tensor_call_args_str}}};")
